...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    func application(_ application: UIApplication, didFinishLaunchingWithOptions launchOptions: [UIApplicationLaunchOptionsKey: Any]?) -> Bool {
        ...
        // Plist configuration needed (but fake fb login)
    ...
    func didConnect(_ success: Bool) {
        if success {
            ...
            switchToOnboardingInterface()
        }
    }
    
    func switchToOnboardingInterface() {
        let controller = OnboardingVC()
        self.window?.rootViewController = controller
    }
    
    func switchToUserInterface() {
        let controller = SmiileTabBarVC()
